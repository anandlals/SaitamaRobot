...
def list_handlers(update, context):
    ...
    if not conn is False:
        ...
        filter_list = "*Filter in slot 2 at ki {}:*\n"
    else:
        ...
        if chat.type == "private":
            ...
            filter_list = "*Filters in slot 2 at {}*:\n"

    all_handlers = sql.get_chat_triggers(chat_id)

    if not all_handlers:
        send_message(update.effective_message,
                     "No filters saved in slot 2 at {}!".format(chat_name))
        return
...
def filters(update, context):
    ...
    if add is True:
        send_message(
            update.effective_message,
            "Slot 2 saved filter '{}' in *{}*!".format(keyword, chat_name),
            parse_mode=telegram.ParseMode.MARKDOWN,
        )
...
def stop_filter(update, context):
    ...
    if not conn is False:
        ...
        chat_id = update.effective_chat.id
        if chat.type == "private":
            chat_name = "Local filters in slot 2"
        else:
            chat_name = chat.title
    ...
    if not chat_filters:
        send_message(update.effective_message, "No filters active here! in slot 2")
        return
    ...
    for keyword in chat_filters:
        if keyword == args[1]:
            ...
            send_message(
                update.effective_message,
                "Okay, I'll stop replying to that filter in slot 2 *{}*.".format(
                    chat_name),
                parse_mode=telegram.ParseMode.MARKDOWN,
            )
            raise DispatcherHandlerStop
    ...
    send_message(
        update.effective_message,
        "That's not a filter in slot 2 - Click: /filters to get currently active filters.",
    )
    ...
    if member.status != "creator" and user.id not in DRAGONS:
        ...
        buttons = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                text="Stop all filters in slot 2", callback_data="filters_rmall")
        ], [
...
__help__ = """
 • `/filters`*:* List all active filter slot 2 filters saved in the chat.

*Admin only:*
 • `/filter2 <keyword> <reply message>`*:* Add a filter to this chat. The bot will now reply that message whenever 'keyword'\
is mentioned. If you reply to a sticker with a keyword, the bot will reply with that sticker. NOTE: all filter \
keywords are in lowercase. If you want your keyword to be a sentence, use quotes. eg: /filter2 "hey there" How you \
doin?
 ...
 *Example:* 
 ...
 %%%
 Reply 3`
 • `/stop2 <filter keyword>`*:* Stop that filter from filter slot 2.

*Chat creator only:*
 • `/removeallfilters2`*:* Remove all chat filters slot 2 at once.

*Note*: Filters slot 2 is also support markdown formatters like: {first}, {last} etc.. and buttons.
Check `/markdownhelp` to know more!

"""

__mod_name__ = "Filters 2"

FILTER_HANDLER = CommandHandler("filter2", filters)
STOP_HANDLER = CommandHandler("stop2", stop_filter)
RMALLFILTER_HANDLER = CommandHandler(
    "removeallfilters2", rmall_filters, filters=Filters.group)
RMALLFILTER_CALLBACK = CallbackQueryHandler(
    rmall_callback, pattern=r"filters_.*")
LIST_HANDLER = DisableAbleCommandHandler(
    "filters2", list_handlers, admin_ok=True)
